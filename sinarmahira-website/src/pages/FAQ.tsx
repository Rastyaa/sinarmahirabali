        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-[48px]">
          ...
          <div className="lg:col-span-2 space-y-[16px]">
            {faqData.map((faq, index) => (
              <div 
                key={index} 
                className={`bg-white rounded-[16px] border ${openIndex === index ? 'border-brand-coral' : 'border-border-subtle'} overflow-hidden shadow-sm transition-colors`}
              >
                <button 
                  onClick={() => toggleAccordion(index)}
                  className="w-full flex items-center justify-between p-[24px] text-left focus:outline-none"
                >
                  <span className={`text-[18px] font-bold font-poppins ${openIndex === index ? 'text-brand-coral' : 'text-text-primary'}`}>
                    {faq.question}
                  </span>
                  <ChevronDown 
                    className={`w-[24px] h-[24px] text-text-muted transition-transform duration-300 ${openIndex === index ? 'rotate-180 text-brand-coral' : ''}`} 
                  />
                </button>
                <div 
                ...
                >
                  <p className="text-[16px] text-text-secondary leading-[32px] font-jakarta border-t border-border-subtle pt-[16px]">
                    {faq.answer}
                  </p>
                </div>
              </div>
          ...
          <div className="space-y-[32px]">
             <div className="bg-white border-t-[4px] border-t-brand-blue rounded-[16px] p-[32px] shadow-raised text-center flex flex-col items-center">
               ...
               <Link 
                 to="/contact" 
                 className="flex items-center justify-center gap-[8px] w-full bg-surface-light border border-border-subtle text-text-primary h-[48px] rounded-[12px] font-bold hover:bg-brand-blue hover:text-white hover:border-transparent transition-colors"
               >
